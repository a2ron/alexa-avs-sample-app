...
public class AVSController implements RecordingStateListener, AlertHandler, AlertEventListener, AccessTokenListener,
        DirectiveDispatcher, AlexaSpeechListener, ParsingFailedHandler, UserActivityListener, WakeWordDetectedHandler {
    ...
    public AVSController(AVSAudioPlayerFactory audioFactory, AlertManagerFactory alarmFactory,
        ...
        pin = gpio.provisionDigitalOutputPin(RaspiPin.GPIO_15, "PinLED", PinState.LOW);
        ...
        directiveEnqueuer = new DirectiveEnqueuer(dialogRequestIdAuthority, dependentQueue, independentQueue, this);
        ...
        independentDirectiveThread = new BlockableDirectiveThread(independentQueue, this, "IndependentDirectiveThread");
    ...
    public void init(ListenHandler listenHandler, NotificationIndicator notificationIndicator,
        ...
        this.notificationManager = new NotificationManager(notificationIndicator, new SimpleAudioPlayer());
    ...
    private void getMicrophone(AVSController controller) throws LineUnavailableException {
        controller.microphone = AudioCapture.getAudioHardware(AUDIO_TYPE.getAudioFormat(), new MicrophoneLineFactory());
    }
    ...
    private void initializeMicrophone() {
        ...
        if (this.wakeWordAgentEnabled) {
            ...
            try {
                LinearRetryPolicy retryPolicy = new LinearRetryPolicy(WAKE_WORD_RELEASE_RETRY_DELAY_MS,
    ...
    public void sendSynchronizeStateEvent() throws InterruptedException {
        sendRequest(RequestFactory.createSystemSynchronizeStateEvent(player.getPlaybackState(), player.getSpeechState(),
                alertManager.getState(), player.getVolumeState(), notificationManager.getState()));
    }
    ...
    public void startRecording(RecordingRMSListener rmsListener, RequestListener requestListener) {
        ...
        try {
            ...
            RequestBody body = RequestFactory.createSpeechRecognizerRecognizeRequest(dialogRequestId, PROFILE, FORMAT,
    ...
    public void handlePlaybackAction(PlaybackAction action) {
        ...
        case PLAY:
            if (alertManager.hasActiveAlerts()) {
                ...
                sendRequest(RequestFactory.createPlaybackControllerPlayEvent(player.getPlaybackState(),
        ...
        case PAUSE:
            if (alertManager.hasActiveAlerts()) {
                ...
                sendRequest(RequestFactory.createPlaybackControllerPauseEvent(player.getPlaybackState(),
                        player.getSpeechState(), alertManager.getState(), player.getVolumeState()));
            }
            break;
        case PREVIOUS:
            sendRequest(RequestFactory.createPlaybackControllerPreviousEvent(player.getPlaybackState(),
                    player.getSpeechState(), alertManager.getState(), player.getVolumeState()));
            break;
        case NEXT:
            sendRequest(RequestFactory.createPlaybackControllerNextEvent(player.getPlaybackState(),
                    player.getSpeechState(), alertManager.getState(), player.getVolumeState()));
            break;
        default:
    ...
    public synchronized void dispatch(Directive directive) {
        ...
        try {
            ...
            sendExceptionEncounteredEvent(directive.getRawMessage(), ExceptionType.INTERNAL_ERROR, e);
            throw e;
        }
    }

    private void sendExceptionEncounteredEvent(String directiveJson, ExceptionType type, Exception e) {
        sendRequest(RequestFactory.createSystemExceptionEncounteredEvent(directiveJson, type, e.getMessage(),
                player.getPlaybackState(), player.getSpeechState(), alertManager.getState(), player.getVolumeState()));
    ...
    private void handleSpeechRecognizerDirective(Directive directive) throws DirectiveHandlingException {
        ...
        } else if (AVSAPIConstants.SpeechRecognizer.Directives.StopCapture.NAME.equals(directiveName)) {
    ...
    private void handleNotificationsDirective(Directive directive) throws DirectiveHandlingException {
        ...
        } else if (AVSAPIConstants.Notifications.Directives.ClearIndicator.NAME.equals(directiveName)) {
    ...
    private void handleTemplateRuntimeDirective(Directive directive) throws DirectiveHandlingException {
        ...
        if (AVSAPIConstants.TemplateRuntime.Directives.RenderTemplate.NAME.equals(directiveName)) {
            cardHandler.renderCard((RenderTemplate) directive.getPayload(), directive.getRawMessage());
        } else if (AVSAPIConstants.TemplateRuntime.Directives.RenderPlayerInfo.NAME.equals(directiveName)) {
    ...
    private void throwUnsupportedOperationException(Directive directive) throws DirectiveHandlingException {
        throw new DirectiveHandlingException(ExceptionType.UNSUPPORTED_OPERATION,
                String.format("The device's %s component cannot handle the %s directive", directive.getNamespace(),
                        directive.getName()));
    }
    ...
    private void handleSetEndpoint(SetEndpoint setEndpoint) throws DirectiveHandlingException {
        try {
            ...
            throw new DirectiveHandlingException(ExceptionType.INTERNAL_ERROR, "Error while creating avsClient");
        }
    }
    ...
    public void processingFinished() {
        ...
        speechRequestAudioPlayerPauseController.speechRequestProcessingFinished(dependentQueue.size());
    }
    ...
    private class UserInactivityReport implements Runnable {
        ...
        @Override
        public void run() {
            sendRequest(RequestFactory
                    .createSystemUserInactivityReportEvent((System.currentTimeMillis() / MILLISECONDS_PER_SECOND)
